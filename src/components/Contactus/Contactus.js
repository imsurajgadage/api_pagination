import React,{useState} from "react";
import validator from "validator";
import styles from "./Contactus.module.css";
import SearchIcon from "@material-ui/icons/Search";

const Contactus = () => {
  const [isEmail, setIsEail] = useState(false)
  const [isNumber,setIsNumber] = useState(false)

  const validateEmail = (e) => {
    var email = e.target.value
  
    if (validator.isEmail(email)) 
    setIsEail(true)
     else 
     setIsEail(false)
  }

  const validateNumber = (e) => {
    var phone = e.target.value
  
    if (validator.isMobilePhone(phone)) 
    setIsNumber(true)
     else 
     setIsNumber(false)
  }

  return (
    <div className={styles.container}>
      <div className={styles.profile}>
        <div className={styles.card}>
          <img
            src="data:image/jpeg;base64,"
            alt="Profile"
            className={styles.card_avatar}
          />
          <div className={styles.content}>
            <p className={styles.reader}>Hii Readers</p>
            <p>Here's your News!</p>
          </div>
        </div>

        {/* Have a FeedBack */}
        <div className={styles.card3}>
          <h2>Have a Feedback?</h2>
          <button className={styles.contact_btn}>We're Listening</button>
        </div>
      </div>
      <div className={styles.formDiv}> 
        <h1>Thank you So much for taking the time</h1>
        <p>Please Provide the below details!</p>
        <label>First Name:</label>
        <input type="text" className={styles.details} placeholder="John"/>
        <label>Last Name:</label>
        <input type="text" className={styles.details} placeholder="Doe"/>
        <label>Address:</label>
        <textarea rows="5" cols="10" placeholder="Enter your full Postal Address"/>
        <label>Country:</label>
        <div>
          <input type="text" className={styles.details} placeholder="India"/>  
          <SearchIcon className={styles.searchIcon}/>
        </div>
        
        <label>Email Id:</label>
        <div>
          <input type="email" placeholder="smaple@example.com" className={styles.details}
           onChange={(e) => validateEmail(e)}
          />
          {isEmail ? null : "Please Enter a valid e-mail"}
        </div>  
        <label>Phone Number</label>
        <div>
          <input type="number" placeholder="91" className={styles.code}/>
          <input type="number" placeholder="1234567890" className={styles.phone}
          onChange={(e) => validateNumber(e)}
          />
          {isNumber ? null : "Please enter a valid number"}
        </div>
        <button className={styles.button}>Submit FeedBack</button>
       
      </div>


    </div>
  );
};

export default Contactus;

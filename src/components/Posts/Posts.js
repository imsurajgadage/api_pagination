import React from 'react'
import styles from './Posts.module.css'
import { useHistory } from "react-router";

const Posts = ({data}) => {
    const history = useHistory()

    function clickHandler() {
        history.push("/contact");
      }

    return (
        <div className={styles.container}>
        <div className={styles.profile}>
          <div className={styles.card}>
            <img
              src="data:image/jpeg;base64,"
              alt="Profile"
              className={styles.card_avatar}
            />
            <div className={styles.content}>
              <p className={styles.reader}>Hii Readers</p>
              <p>Here's your News!</p>
            </div>
          </div>
  
          {/* Toggle View */}
  
          <div className={styles.card2}>
            <h2>View Toggle</h2>
          </div>
  
          {/* Have a FeedBack */}
          <div className={styles.card3}>
            <h2>Have a Feedback?</h2>
            <button onClick={clickHandler} className={styles.contact_btn}>We're Listening</button>
          </div>
        </div>
        <div className={styles.newsContainerDiv}>
          {data.map((news) => (
            <div className={styles.newsDiv} key={news.id}>
              <img src={news.link} alt="Link" className={styles.card_avatar} />
  
              <div className={styles.subNewsDiv}>
                <p className={styles.title}>{`${news.title.substring(
                  0,
                  100
                )}...`}</p>
                <p className={styles.summary}>{`${news.title.substring(
                  0,
                  100
                )}...`}</p>
                <p className={styles.published}>{`${news.title.substring(
                  0,
                  100
                )}...`}</p>
                <button className={styles.cross_button}>X</button>
              </div>
            </div>
          ))}
        </div>
      </div>
    )
}

export default Posts
